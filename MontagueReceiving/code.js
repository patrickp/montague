...
function AddReceipt() {
    ...
    receipt.lastreceipt = "last";
    receipt.pOnO = "";
    receipt.type = "N";
    receipt.packingSlip = "";
    receipt.notes="";
    receipt.postDate = CDate(new Date());
...
function UpdateReceiptForm() {
 // This will set us up for a new receipt
  ...
  ReceiptFormReceiptDate.value          = FormatDateTime(receipt.postDate,10);
  ReceiptFormType.value                 = receipt.type;
  ReceiptFormPackingSlipNo.value        = receipt.packingSlip;


  SummaryTable.clear();
  SummaryColumns = [{"title":"LI#" , "width":"15px"} ,{"title":"P#" ,"width":"15px"},{"title":"PART#" ,"width":"120px" , "className":"nowrap"},{"title":"Description"},{"title":"ORD" ,"width":"100px" , "className":"text-right"},{"title":"Recvd" ,"width":"100px" , "type":"num" , "className":"text-right"}];
  ...
  for     (x=(1); x <= NumItems; x++) {
    ...
    row = [ x, item.pOLineNo, item.partNo, item.desc, item.qtyOrd, item.qtyRec ];
    console.log(row);
...
SummaryButton3.onclick = function() {
  NSB.MsgBox("This will do a save action and return to search screen");
};
...
SummarySubBtnReceiveAll.onclick = function() {
  SummarySubReceived.value = SummarySubOrdered.value;
  CalcBo();
};
...
function loadreceiptitem(pos) {
   ...
   SummarySubDescription.value          = item.desc;
